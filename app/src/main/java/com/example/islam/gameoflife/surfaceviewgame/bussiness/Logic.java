...
import com.example.islam.gameoflife.dagger.logic.DaggerLogicComponent;
import com.example.islam.gameoflife.dagger.logic.LogicModule;
import com.example.islam.gameoflife.models.Cell;
import com.example.islam.gameoflife.models.Touch;
 ...
 * Conway's GamePresenter of Life. The game mechanics are described here:
 * http://en.wikipedia.org/wiki/Conway's_Game_of_Life
 *
 * @author Tomas Varaneckas
 */
public class Logic {
    private int gamMatrixWidth, gamMatrixHeight;
    @Inject
    Input input;
    ...
    public Logic(GameContext context) {
        ...
        DaggerLogicComponent.builder().applicationComponent(context.getApplicationComponent()).logicModule(new LogicModule()).build().inject(this);
     ...
     * running the logic.
     */
    public Observable<Collection<Cell>> tick() {


        return Observable.interval(0, UIConstants.PERIODIC_INTERVAL, TimeUnit.MILLISECONDS, Schedulers.newThread()).map(aLong -> {
            if (context.getState() == GameContext.State.RUNNING) {
                // Materialize user touches into cells.
                Set<Touch> touches = input.getProcessed();
            ...
            for (Touch touch : input.getUnprocessed()) {
    ...
    /**
     * The main logic that implements Conway's GamePresenter of Life.
     * <p>
...
//			Utils.debug(this, "GamePresenter logic took: %s", (end - start));
//		}
    }
    ...
    /**
     * According to Conway's GamePresenter of Life rule #4, if a dead cell has exactly
    ...
    public int getGamMatrixWidth() {
        return gamMatrixWidth;
    }

    public void setGamMatrixWidth(int gamMatrixWidth) {
        this.gamMatrixWidth = gamMatrixWidth;
    }

    public int getGamMatrixHeight() {
        return gamMatrixHeight;
    }

    public void setGamMatrixHeight(int gamMatrixHeight) {
        this.gamMatrixHeight = gamMatrixHeight;
    }
    ...
    public void addTouch(float x, float y, float pressure) {
        input.addTouch(x, y, pressure);
    }

    public Set<Touch> getUnprocessed() {
        return input.getUnprocessed();
    }
}